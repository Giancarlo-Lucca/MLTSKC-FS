            ...
            for j=1:length(beta_searchrange) 
                for k = 1:length(gamma_searchrange) 
                    ...
                    if r == 1
                        ...
                        BestParameter.model_LLSF = model_LLSF;
                        save('BestModel.mat', 'model_LLSF', 'optmParameter', 'TSKoptions');
                    end
                    rx(q_s,index-1) = index-1;                    
                    ry1(q_s,index-1) = Result(12,1);
                    ry2(q_s,index-1) = Result(1,1);
                    ry3(q_s,index-1) = Result(13,1);
                    ry4(q_s,index-1) = Result(14,1);
                    ry5(q_s,index-1) = Result(15,1);
                end
            end
        end
    end
...
figure;
q = q_s;
plot(rx(q,:),ry1(q,:),rx(q,:),ry2(q,:));
% Establecer el límite inferior del eje y en 0.5
ylim([0.0, 1]);
